...
# Initialize Spark
spark = SparkSession.builder.appName("Delete Logic Full Fabric").getOrCreate()
...
def process_table(row_dict):
    ...
    try:
        ...
        try:
            ...
            target_df = spark.createDataFrame([], spark.read.parquet(f"{staging_path}/{stg_folder}").schema).alias("tgt")
        ...
        delete_df = filtered_full_df.join(
        ...
        update_log(table_name + "_DELETE", "SUCCESS", start_time, end_time)

    except Exception as e:
        end_time = datetime.now()
        error_message = traceback.format_exc()
        print(f"FAILURE for {table_name}: {e}")
        update_log(table_name + "_DELETE", "FAILURE", start_time, end_time, error_message)

# ✅ Smarter decide which tables to run for DELETE
try:
    main_log_df = spark.read.parquet(main_log_path)
...
print("✅ Delete process with smart skip/retry completed successfully!")
